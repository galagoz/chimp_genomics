...
outDir="/data/workspaces/lag/workspaces/lg-evolution/analysis/tai_chimp_genomics/results/variant_calling/post-variantCalling_QC_indels"
logDir="${outDir}/logs"
...
for SAMPLE in "${samples[@]}"; do
  INPUT_VCF="${inDir}/${SAMPLE}.filtered.indels.vcf.gz"
  ...
  JOB_SCRIPT="${logDir}/job_${SAMPLE}.sh"
    ...
    cat <<EOF > "$JOB_SCRIPT"
#!/bin/bash
#$ -N indelQC_${SAMPLE_ID}
#$ -cwd
#$ -q multi15.q
#$ -S /bin/bash
#$ -o ${logDir}/indelQC_${SAMPLE}.out     # STDOUT log
#$ -e ${logDir}/indelQC_${SAMPLE}.err     # STDERR log
